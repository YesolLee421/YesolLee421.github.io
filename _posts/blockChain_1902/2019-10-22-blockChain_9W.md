...
## 블록 쌓기
### 1. 블록체인-데이터-구조의 특징
- 블록체인-데이터-구조의 맨 끝에 새 블록 추가는 쉬움
- 중간에 있는 블록 내용 수정은 거의 불가능
- 중간 블록 내용 변경을 감지하고 원본으로 되돌리는 메카니즘(mechanism) 포함<br>

### 2. 최초의 블록체인-데이터-구조
- 블록 당 2개의 거래 정보(트랜잭션)를 수용하는 경우
- 트랜잭션 1, 2가 첫 번째 블록에 저장됨
- 그 후 새 블록 추가 과정
    1. 한 블록 내에 들어갈 거래 정보를 모아 머클트리 생성(거래 3, 4 각각 해시 후 합쳐서 다시 해시->머클트리 루트값 생성: 거래 3, 4 전체에 대한 정보)
    2. 이전 블록헤더를 해시한 값을 해시 참조로 사용
    3. 블록헤더 생성 = 머클 트리(해당 블록의 전체 거래내용) + 이전블록 가리키는 해시참조
    4. 생성된 블록헤더를 가리키는 해시참조 생성 = 블록체인 데이터 구조 전체에 대한 포인터(마지막 블록헤더 가리키는 주소값)<br>

## 변경의 감지
### 1. 블록체인-데이터-구조의 일부가 변경된 경우
1. 거래 정보 내용 불일치
    - 그 거래 정보(개별 트랜잭션)의 hash참조와 불일치
2. 거래 정보의 hash참조를 변경하면?
    - 머클 트리의 루트 해시값과 불일치
3. 머클 트리의 루트를 변경하면?
    - 다음 블록 안에 있는 이전 블록의 hash참조와 불일치
4. 다음 블록 안의 이전 블록에 대한 hash참조를 변경하면?
    - 블록체인-데이터구조 전체에 대한 참조와 불일치<br>

### 2. 블록체인-데이터-구조의 변경감지
- 거래 정보의 일부가 변경 -> 이를 포함한 블록 및 이후에 연결된 모든 블록 수정해야 함
- 블록 하나 생성 시 약 10분 걸릴 정도로 계산 자원 많이 필요
- 즉 거의 불가능에 가까움 => 일관성, 무결성 유지 가능<br>

### 3. 의도한 변경 VS 의도하지 않은 변경 차이는?
- 블록체인-데이터-구조는 변경 내용 빠르게 감지 가능
- 변경 여부 확인에 hash참조 이용해서 아주 작은 변화도 쉽게 감지
- 그러나 변경의 의도는 확인할 수 없다.<br>



![알기쉬운블록체인_마인드맵_9주차](https://user-images.githubusercontent.com/54266900/67553346-ef3cf900-f747-11e9-9751-b3dfd3e7fe90.jpg)
